    ...
    <header className="sticky top-0 z-50 w-full px-4 sm:px-8 py-4 sm:py-6">
      <div className="glass h-20 rounded-[2rem] px-4 sm:px-8 flex items-center justify-between shadow-2xl shadow-black/5 dark:shadow-black/20 border border-white/40 dark:border-white/5 relative">
        ...
        <div className="flex items-center gap-3 shrink-0">
          <div className="w-10 h-10 bg-primary-600 rounded-xl flex items-center justify-center shadow-lg shadow-primary-500/30 rotate-3 transition-transform hover:rotate-0">
        ...
        {/* Desktop Menu */}
        <nav className="hidden lg:flex items-center gap-1 bg-slate-100/50 dark:bg-white/5 p-1.5 rounded-2xl mx-4">
          {menuItems.map((item) => {
        ...
        <div className="flex items-center gap-2 sm:gap-4 ml-auto sm:ml-0">
          ...
          <button 
            ...
            className="lg:hidden p-2.5 bg-white dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/10 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-white/10 transition-all active:scale-95"
          >
            {isMobileMenuOpen ? <X size={20} className="animate-in spin-in-90 duration-300" /> : <Menu size={20} className="animate-in zoom-in duration-300" />}
          </button>
        </div>

        {/* Mobile Menu Dropdown */}
        {isMobileMenuOpen && (
          <div className="absolute top-[calc(100%+1rem)] left-0 right-0 glass p-6 rounded-[2.5rem] border border-white/40 dark:border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.15)] lg:hidden animate-in fade-in slide-in-from-top-4 duration-500 z-50">
              ...
              {menuItems.map((item, idx) => {
                ...
                return (
                  <Link
                    ...
                    className={`flex flex-col items-center gap-3 p-5 rounded-[2rem] transition-all duration-300 ${
                      isActive 
