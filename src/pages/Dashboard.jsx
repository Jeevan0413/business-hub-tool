    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {[
          ...
          <Card key={i} className="group hover:border-primary-500/50 transition-all cursor-default p-6 sm:p-8">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className={`text-[10px] font-black px-2.5 py-1 rounded-full ${stat.color} bg-slate-100 dark:bg-white/5 uppercase tracking-wider`}>
                {stat.change}
              </span>
            </div>
            <p className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 font-black uppercase tracking-[0.15em] mb-1">{stat.label}</p>
            <h3 className="text-2xl sm:text-3xl font-black dark:text-white">{stat.value}</h3>
          </Card>
        ))}
      </div>
      ...
      <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {tools.map((tool) => (
          <motion.div key={tool.name} variants={item}>
            <Link to={tool.path}>
              <Card className="h-full flex flex-col group relative overflow-hidden hover:border-primary-500/50 transition-all active:scale-[0.98] p-6 sm:p-8">
                ...
                <div className="flex flex-col gap-4 relative z-10 h-full">
                  ...
                  <div>
                    <h3 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white mb-2">{tool.name}</h3>
                    <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 leading-relaxed">{tool.description}</p>
                  </div>
                  <div className="mt-auto flex items-center gap-2 text-primary-600 text-[10px] sm:text-xs font-black group-hover:translate-x-1 transition-all pt-4 uppercase tracking-widest">
